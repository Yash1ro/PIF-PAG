...
namespace hnswlib {
    ...
    public:
        ...
        HierarchicalNSW(size_t maxk, int step, size_t m, size_t vecdim, std::vector<std::vector<std::vector<float>>>& projVec, int level, int subdim, std::vector<int>& permutation, SpaceInterface<dist_t> *s1, SpaceInterface<dist_t> *s2, SpaceInterface<dist_t> *s3, SpaceInterface<dist_t> *s4, 
		        size_t max_elements, char* path_index, size_t m0, std::vector<std::vector<std::vector<float>>>& projVec0, std::vector<int>& permutation0, size_t tablesize, size_t M = 16, size_t ef_construction = 200) :
                link_list_locks_(max_elements), element_levels_(max_elements), proj_locks_(16*m0*m0*m0*m0) {
            
			proj_flags.resize(16*m0*m0*m0*m0);
            ...
            int subdim0 = extended_dim_ / 4;

            for (int s = 0; s < 4; ++s) {
                for (int l = 0; l < m0_; ++l) {
                    float* dst = projVecQuery0 + (s * m0_ + l) * subdim0;

                    for (int k = 0; k < subdim0; ++k) {
                        dst[k] = projVec0[s][l][k];
                    }
                }
            }			
			...
			size_links_level0_ = maxM0_ * (2 * sizeof(int16_t) + sizeof(tableint) + half_level) + sizeof(linklistsizeint);   
        ...
        std::vector<std::mutex> proj_locks_;		
		
		std::vector<float> proj_flags;
        ...
        inline unsigned char* getQuanVal(char* data, int i, int j) const { 
			int a = i / 16;
			int b = i % 16; 			
    ...
    inline void insertIntoInitialTableColumn(Node* col_base, const float score, const int new_id) noexcept {
        if (score <= col_base[tablesize_ - 1].value) return;

        int pos = tablesize_ - 1;
        while (pos > 0 && score > col_base[pos - 1].value) {
            col_base[pos] = col_base[pos - 1];
            --pos;
        }
        col_base[pos].value = score;
        col_base[pos].index = new_id;
    }
...
__m256i bf16_0 = _mm512_castsi512_si256(bf16_01);                 
__m256i bf16_1 = _mm512_extracti64x4_epi64(bf16_01, 1);           
...
_mm512_store_ps(Thres, sum);
        ...
        std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst>
        searchBaseLayerFast2(tableint cur_c, const void *query_point_tmp, std::vector<Node>& InitialTable){
            ...
            float* query_extended_point0 = new float[extended_dim_];
			permute0(query_extended_point0, cur_query_point);

            VisitedList *vl = visited_list_pool_->getFreeVisitedList();
            vl_type *visited_array = vl->mass;
            vl_type visited_array_tag = vl->curV;
			
			int max_round2 = ef_construction_ / step;  
...
std::vector<tableint> seed_ids(step);

{
    std::lock_guard<std::mutex> lock(proj_locks_[col_id]);
    Node* col_base = InitialTable.data() + col_id * static_cast<size_t>(tablesize_);
    for (int i = 0; i < step; ++i) {
        seed_ids[i] = col_base[i].index;
    }
}


for (int i = 0; i < step; ++i) {
    int cur_id = seed_ids[i];
    float* norm_pointer = (float*)getNormByInternalId(cur_id);
    ...
    retset[i] = NeighborIndex(cur_id, dist, ip, true);
...
std::sort(retset.begin(), retset.begin() + step);
    ...
    for (int p01 = 0; p01 < pair_cols; ++p01) {
        const float left = pair01[p01];

        for (int p23 = 0; p23 < pair_cols; ++p23) {
            const size_t update_col_id =
                static_cast<size_t>(p01) * pair_cols + static_cast<size_t>(p23);

            const float score = left + pair23[p23];
		    ...
		    if (score <= proj_flags[update_col_id]) {
                continue;
            }

            std::unique_lock<std::mutex> lock(proj_locks_[update_col_id]);
            ...
            Node* col_base =
                InitialTable.data() + update_col_id * static_cast<size_t>(tablesize_);
		
            insertIntoInitialTableColumn(col_base, score, cur_c);
            proj_flags[update_col_id] = col_base[tablesize_ - 1].value;
...
_mm512_store_ps(Thres, sum);
					...
					if(reverse_flag == true){
						insert_neighbor(addNeighbors, n, cur_c, true_qnorm*cur_dist);
					}
...
Node* InitialNode = InitialTable.data() + col_id * static_cast<size_t>(tablesize_);
...
sum = _mm512_mul_ps(sum, v_mul);
        ...
        void saveIndex() {
            ...
            writeBinaryPOD(output, tablesize_);
        ...
        void loadIndex(const char* path_index, SpaceInterface<dist_t> *s1, SpaceInterface<dist_t> *s2, SpaceInterface<dist_t> *s3, SpaceInterface<dist_t> *s4, size_t max_elements_i=0) {
            ...
            readBinaryPOD(input, tablesize_);
        ...
        void addPoint(const void *data_point, labeltype label, float* norm, std::vector<Node>& InitialTable) {
			...
			*scale_pointer = 32767.0f / find_max_abs(vec_normalized);			
            ...
            if (cur_c > 0) {
				...
				if(cur_c < tablesize_)
                    ...
                    top_candidates = searchBaseLayerFast2(cur_c, data_point, InitialTable);	
				}
...
void buildTable(std::vector<Node>& InitialTable, float** init_points) {
    const int npts = tablesize_;
    const int signed_m = 2 * m0_;
    const int pair_cols = signed_m * signed_m;         
    const size_t table_cols =
        static_cast<size_t>(pair_cols) * pair_cols;    
    const int subdim = extended_dim_ / 4;

    
    InitialTable.resize(table_cols * static_cast<size_t>(tablesize_));
    proj_flags.resize(table_cols);

    
    
    
    std::vector<float> permuted_points(static_cast<size_t>(npts) * extended_dim_, 0.0f);

#pragma omp parallel for schedule(static)
    for (int i = 0; i < npts; ++i) {
        float* dst = permuted_points.data() + static_cast<size_t>(i) * extended_dim_;
        const float* src = init_points[i];
        for (int j = 0; j < extended_dim_; ++j) {
            dst[j] = src[permutation0_[j]];
        }
    }
    ...
    std::vector<float> projValT(static_cast<size_t>(4) * signed_m * npts);

#pragma omp parallel for schedule(static)
    for (int i = 0; i < npts; ++i) {
        const float* x = permuted_points.data() + static_cast<size_t>(i) * extended_dim_;
        ...
        for (int s = 0; s < 4; ++s) {
            ...
            for (int l = 0; l < m0_; ++l) {
                ...
                projValT[(static_cast<size_t>(s) * signed_m + l) * npts + i] = val;
                projValT[(static_cast<size_t>(s) * signed_m + (m0_ + l)) * npts + i] = -val;
            }
        }
    }
    ...
    std::vector<float> pair01T(static_cast<size_t>(pair_cols) * npts);
    std::vector<float> pair23T(static_cast<size_t>(pair_cols) * npts);

#pragma omp parallel for collapse(2) schedule(dynamic)
    for (int a = 0; a < signed_m; ++a) {
        for (int b = 0; b < signed_m; ++b) {
            const size_t pid = static_cast<size_t>(a) * signed_m + b;
            const float* col0 = projValT.data() + (static_cast<size_t>(0) * signed_m + a) * npts;
            const float* col1 = projValT.data() + (static_cast<size_t>(1) * signed_m + b) * npts;
            float* out = pair01T.data() + pid * npts;
...
#pragma omp parallel for collapse(2) schedule(dynamic)
    for (int c = 0; c < signed_m; ++c) {
        for (int d = 0; d < signed_m; ++d) {
            ...
            const float* col2 = projValT.data() + (static_cast<size_t>(2) * signed_m + c) * npts;
            const float* col3 = projValT.data() + (static_cast<size_t>(3) * signed_m + d) * npts;
            float* out = pair23T.data() + pid * npts;
...
#pragma omp for collapse(2) schedule(dynamic)
        for (int p01 = 0; p01 < pair_cols; ++p01) {
            for (int p23 = 0; p23 < pair_cols; ++p23) {
                ...
                const float* left = pair01T.data() + static_cast<size_t>(p01) * npts;
                const float* right = pair23T.data() + static_cast<size_t>(p23) * npts;
                ...
                Node* out = InitialTable.data() + col_id * static_cast<size_t>(tablesize_);
                std::memcpy(out, buf.data(), static_cast<size_t>(tablesize_) * sizeof(Node));
                proj_flags[col_id] = out[tablesize_ - 1].value;
            }
        }
    }
}

        void compression(int vecsize, std::vector<Node>& InitialTable){
...
#pragma omp parallel for schedule(dynamic)				
            ...
            const size_t total_elements = InitialTable.size();

            #pragma omp parallel for schedule(static)
            for (size_t i = 0; i < total_elements; ++i) {
                InitialTable[i].index = convert_id2[InitialTable[i].index];
            }
